...
# Node Records - A records for each Redis Enterprise cluster node  
# Creates: node1.name-prefix.domain.com, node2.name-prefix.domain.com, etc.
resource "aws_route53_record" "redis_nodes" {
  count   = var.create_dns_records ? var.node_count : 0
  zone_id = var.dns_hosted_zone_id
  name    = "node${count.index + 1}.${var.name_prefix}.${data.aws_route53_zone.main.name}"
  type    = "A"
  ttl     = 300
...
# A Record for UI access - Points directly to primary node
# Creates: name-prefix.domain.com (matches cluster FQDN)
resource "aws_route53_record" "redis_cluster_ui" {
  count   = var.create_dns_records ? 1 : 0
  zone_id = var.dns_hosted_zone_id
